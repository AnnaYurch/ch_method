...
// Функция для записи данных в файл для построения графиков
void save_plot_data(SolutionPoint *solution, int n, double alpha_final) {
    FILE *file;
    
    // 1. График решения и производной
    file = fopen("solution_plot.dat", "w");
    fprintf(file, "# x    y_численное   y_точное   y_производная\n");
    for (int i = 0; i < n; i++) {
        fprintf(file, "%.6f   %.6f   %.6f   %.6f\n", 
                solution[i].x, solution[i].y_num, 
                solution[i].y_exact, solution[i].z_num);
    }
    fclose(file);
    
    // 2. График погрешности
    file = fopen("error_plot.dat", "w");
    fprintf(file, "# x    погрешность\n");
    for (int i = 0; i < n; i++) {
        fprintf(file, "%.6f   %.6f\n", solution[i].x, solution[i].error);
    }
    fclose(file);
    
    // 3. Данные для метода стрельбы (несколько тестовых alpha)
    file = fopen("shooting_plot.dat", "w");
    fprintf(file, "# alpha   y(4.5)   целевое_значение\n");
    
    ButcherTable table;
    initialize_butcher_tables(&table);
    
    double alpha_values[] = {-50, -25, 0, 10, 20, 30, 40, 50, 60, 70};
    int num_alphas = 10;
    
    for (int i = 0; i < num_alphas; i++) {
        double y_result = shooting_function(alpha_values[i], &table);
        fprintf(file, "%.6f   %.6f   %.6f\n", 
                alpha_values[i], y_result, 79.612);
    }
    fclose(file);
    
    printf("\nДанные для графиков сохранены в файлы:\n");
    printf("- solution_plot.dat: решение и производная\n");
    printf("- error_plot.dat: погрешность\n");
    printf("- shooting_plot.dat: метод стрельбы\n");
}

// Функция для создания скрипта gnuplot
void create_gnuplot_script() {
    FILE *script = fopen("plot_graphs.gnu", "w");
    
    fprintf(script, "set terminal pngcairo size 1200,800 enhanced font 'Arial,10'\n");
    fprintf(script, "set multiplot layout 2,2 title 'Метод стрельбы: x²y'' - xy' + y = 3x³' font 'Arial,12'\n\n");
    
    // График 1: Решение и производная
    fprintf(script, "set title 'Решение и производная'\n");
    fprintf(script, "set xlabel 'x'\n");
    fprintf(script, "set ylabel 'y(x)'\n");
    fprintf(script, "plot 'solution_plot.dat' using 1:2 with linespoints title 'y_численное', \\\n");
    fprintf(script, "     'solution_plot.dat' using 1:3 with lines title 'y_точное', \\\n");
    fprintf(script, "     'solution_plot.dat' using 1:4 with lines title 'y''(x)' axes x1y2\n");
    fprintf(script, "set y2tics\n\n");
    
    // График 2: Погрешность
    fprintf(script, "set title 'Погрешность численного решения'\n");
    fprintf(script, "set xlabel 'x'\n");
    fprintf(script, "set ylabel '|y_числ - y_точн|'\n");
    fprintf(script, "plot 'error_plot.dat' using 1:2 with linespoints title 'Погрешность'\n\n");
    
    // График 3: Метод стрельбы
    fprintf(script, "set title 'Метод стрельбы: F(α) = y(4.5;α) - 79.612'\n");
    fprintf(script, "set xlabel 'α = y'(0.5)'\n");
    fprintf(script, "set ylabel 'y(4.5)'\n");
    fprintf(script, "plot 'shooting_plot.dat' using 1:2 with linespoints title 'y(4.5;α)', \\\n");
    fprintf(script, "     'shooting_plot.dat' using 1:3 with lines title 'Цель: 79.612'\n\n");
    
    // График 4: Сравнение решений с разными alpha
    fprintf(script, "set title 'Сравнение решений с разными α'\n");
    fprintf(script, "set xlabel 'x'\n");
    fprintf(script, "set ylabel 'y(x)'\n");
    fprintf(script, "plot 'solution_plot.dat' using 1:2 with lines title 'y(x) с найденным α', \\\n");
    fprintf(script, "     'solution_plot.dat' using 1:3 with lines title 'Аналитическое решение'\n");
    
    fprintf(script, "unset multiplot\n");
    fprintf(script, "set output 'graphs.png'\n");
    
    fclose(script);
    
    printf("Скрипт для gnuplot создан: plot_graphs.gnu\n");
    printf("Для построения графиков выполните: gnuplot plot_graphs.gnu\n");
}
...
int main() {
    ...
    save_plot_data(solution, n_points, alpha_final);
