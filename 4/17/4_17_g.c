...
// Функция для сохранения данных для графиков
void save_plot_data(int n, double x[MAX_NODES], double y_analytical[MAX_NODES], 
                   double y1[MAX_NODES], double y2[MAX_NODES]) {
    FILE *file = fopen("bvp_solutions.dat", "w");
    if (!file) {
        printf("Ошибка создания файла данных для графиков!\n");
        return;
    }
    
    fprintf(file, "# x Аналитическое Решение_1порядка Решение_2порядка\n");
    for (int i = 0; i < n; i++) {
        fprintf(file, "%.6f %.6f %.6f %.6f\n", 
                x[i], y_analytical[i], y1[i], y2[i]);
    }
    fclose(file);
    printf("Данные для графиков сохранены в файл: bvp_solutions.dat\n");
}

// Функция для создания скрипта GNUplot
void create_gnuplot_script() {
    FILE *script = fopen("plot_bvp.gnu", "w");
    if (!script) {
        printf("Ошибка создания скрипта GNUplot!\n");
        return;
    }
    
    fprintf(script, "set terminal pngcairo size 1200,800 enhanced font 'Arial,12'\n");
    fprintf(script, "set output 'bvp_solution.png'\n");
    fprintf(script, "set title 'Решение краевой задачи №44'\n");
    ...
    fprintf(script, "plot 'bvp_solutions.dat' using 1:2 with lines linewidth 3 title 'Аналитическое решение', \\\n");
    fprintf(script, "     '' using 1:3 with points pointtype 7 pointsize 1.5 title 'Численное (1 порядок)', \\\n");
    fprintf(script, "     '' using 1:4 with points pointtype 9 pointsize 1.5 title 'Численное (2 порядок)'\n");
    
    fclose(script);
    printf("Скрипт GNUplot создан: plot_bvp.gnu\n");
}

// Функция для генерации графиков
void generate_plots() {
    printf("Генерация графиков...\n");
    int result = system("gnuplot plot_bvp.gnu");
    if (result == 0) {
        printf("График сохранен как: bvp_solution.png\n");
        
        // Попробуем открыть график
        int opened = system("xdg-open bvp_solution.png 2>/dev/null");
        if (opened != 0) {
            opened = system("eog bvp_solution.png 2>/dev/null");
        }
        if (opened != 0) {
            printf("График создан. Откройте файл 'bvp_solution.png' вручную.\n");
        }
    } else {
        printf("Ошибка при создании графика! Проверьте установку GNUplot.\n");
    }
}
...
int main() {
    ...
    // Создаем графики
    printf("\nСОЗДАНИЕ ГРАФИКОВ:\n");
    save_plot_data(n, x, y_analytical, y1, y2);
