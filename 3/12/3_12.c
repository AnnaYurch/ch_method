...
// Вывод матрицы
void print_matrix(int n, double A[n][n+1]) {
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n+1; j++) {
            printf("%12.4f ", A[i][j]);
        }
...
// Создание файла данных и построение графика
void plot_graphs(double x[], double y[], double coefs1[], double coefs2[], double coefs3[], int n) {
    ...
    fprintf(data_file, "# x y F1 F2 F3\n");
    for (int i = 0; i < n; i++) {
        fprintf(data_file, "%.6f %.6f %.6f %.6f %.6f\n", 
                x[i], y[i],
                polynomial_value(x[i], coefs1, 1),
                polynomial_value(x[i], coefs2, 2),
                polynomial_value(x[i], coefs3, 3));
    }
    
    // Записываем дополнительные точки для плавных кривых
    for (double xi = -1.0; xi <= 6.5; xi += 0.1) {
        fprintf(data_file, "%.6f %.6f %.6f %.6f %.6f\n", 
                xi, NAN,
                polynomial_value(xi, coefs1, 1),
                polynomial_value(xi, coefs2, 2),
                polynomial_value(xi, coefs3, 3));
    }
    fclose(data_file);
    ...
    fprintf(gnuplot_script, "plot 'data.txt' using 1:2 with points pt 7 ps 1.5 lc rgb 'black' title 'Исходные данные', \\\n");
    fprintf(gnuplot_script, "     'data.txt' using 1:3 with lines lw 2 lc rgb 'red' title 'F1(x) = %.3f + %.3f x', \\\n", coefs1[0], coefs1[1]);
    fprintf(gnuplot_script, "     'data.txt' using 1:4 with lines lw 2 lc rgb 'blue' title 'F2(x) = %.3f + %.3f x + %.3f x^2', \\\n", coefs2[0], coefs2[1], coefs2[2]);
    fprintf(gnuplot_script, "     'data.txt' using 1:5 with lines lw 2 lc rgb 'green' title 'F3(x) = %.3f + %.3f x + %.3f x^2 + %.3f x^3'\n", coefs3[0], coefs3[1], coefs3[2], coefs3[3]);
    fclose(gnuplot_script);
    
    // Запускаем GNUplot
    system("gnuplot plot.gnu");
    printf("\nГрафик сохранен в файл: mnk_graph.png\n");
    printf("Для просмотра выполните: xdg-open mnk_graph.png\n\n");
}

int main() {
    // Данные из варианта 44
